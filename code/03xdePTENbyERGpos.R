...
phenoNatHist$status <- gsub("PTEN_NEGATIVE.ERG_NEGATIVE", "x", phenoNatHist$status)
phenoNatHist$status <- gsub("PTEN_NEGATIVE.ERG_POSITIVE", "0", phenoNatHist$status)
phenoNatHist$status <- gsub("PTEN_POSITIVE.ERG_NEGATIVE", "x", phenoNatHist$status)
...
phenohpfs$status <- gsub("PTEN_NEGATIVE.ERG_NEGATIVE", "x", phenohpfs$status)
phenohpfs$status <- gsub("PTEN_NEGATIVE.ERG_POSITIVE", "0", phenohpfs$status)
phenohpfs$status <- gsub("PTEN_POSITIVE.ERG_NEGATIVE", "x", phenohpfs$status)
...
# Save object
save(xmcmc, file = "./objs/mcmcDeltaTRUE_PTENbyERGpos.rda")
...
#Plot figure
png("figs/PTENinERGPositive/PTENbyERGpos.png", width=1500, height = 2500, res=300)
...
write.csv(merge, file = "../XDE/text/PTENinERGPosxde.csv", row.names = TRUE)
